...
nq.p.squid.I_step= .2e-6;  % current step in amps
...
nq.p.hist.range = .10;    % deviation from the target point for 
                             % registering successful swich from lo -> hi

nq.p.range       = 10; % options: 0.1, 0.2, 0.5, 1, 5, 10

nq.notes = 'testing histogram code';
...
title({['param = ', CSUtils.parsefnameplot(nq.lastparamsave)], ...
       ...
       ', modstep = '        num2str(nq.p.mod.I_step),              ...
